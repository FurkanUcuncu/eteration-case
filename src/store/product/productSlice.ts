...
export const productSlice = createSlice({
  ...
  reducers: {
    ...
    addToCart(state, action) {
      if (
        state.cart.filter((item) => item.name === action.payload.name).length >
        0
      ) {
    ...
    toggleFavorites(state, action) {
      if (
        state.favorites.filter((item) => item.id === action.payload.id).length >
        0
      ) {
    ...
    filterProducts(state, { payload }) {
      ...
      if (minPrice !== "") {
        state.products = state.products.filter(
          (item) => item.price > parsedMinPrice,
        );
      }
      if (maxPrice !== "") {
        state.products = state.products.filter(
          (item) => item.price < parsedMaxPrice,
        );
      }
    },
  },
